...
def _model_map():
    return {
        ...
        "inputs": {
            ...
            "scenario_selector": {"aliases": ["scenario", "kich ban", "base", "best", "worst"], "editable": True, "min": 1, "max": 3},
        },
        "outputs": {
...
def test_scenario_selector_number_and_words():
    assert _changes("chon scenario so 1")["scenario_selector"] == 1
    assert _changes("chay best case")["scenario_selector"] == 2
    assert _changes("worst")["scenario_selector"] == 3
