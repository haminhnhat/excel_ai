...
def _scenario_selector_value(text: str) -> int | None:
    for word, value in SCENARIO_WORD_VALUES.items():
        if word in text:
            return value
    m = re.search(r"(?:scenario|option|case|kich ban|chon|chay|so)\s*(?:so)?\s*([123])\b", text)
    if m:
        return int(m.group(1))
    m = re.search(r"\b([123])\b", text)
    if m and any(token in text for token in ["scenario", "option", "case", "kich ban", "chon", "chay", "so"]):
        return int(m.group(1))
    return None
...
def _change_from_segment(param: str, segment: str, meta: Dict[str, Any] | None = None) -> Change | None:
    ...
    meta = meta or {}
    if param == "scenario_selector":
        value = _scenario_selector_value(segment)
        if value is None:
            return None
        return Change(parameter=param, value=value, operation="set", reason="Parsed scenario selector.")
...
def _parse_with_aliases(command: str, model_map: Dict[str, Any]) -> list[Change]:
    ...
    if "scenario_selector" in model_map.get("inputs", {}):
        scenario_value = _scenario_selector_value(text_norm)
        if scenario_value is not None:
            _add_or_replace(
                ...
                Change(
                    ...
                    operation="set",
                    reason="Parsed scenario selector.",
                ),
            )
