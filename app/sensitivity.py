...
def preview_sensitivity_range(
    ...
    try:
        ...
        clipped = row_count > max_rows or col_count > max_cols
        ...
        rows: list[list[Any]] = []
        for row in range(min_row, max_row + 1):
            rows.append([ws.cell(row=row, column=col).value for col in range(min_col, max_col + 1)])
        return {
            "sheet": sheet_name,
            "range_address": range_address,
            "rows": rows,
            ...
            "displayed_col_count": len(rows[0]) if rows else 0,
            "clipped": clipped,
        }
    finally:
        wb.close()
...
def preview_sensitivity_by_prompt(
    source_path: Path,
    prompt: str,
    max_rows: int = 80,
    max_cols: int = 30,
