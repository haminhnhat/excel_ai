...
if exist "scripts\create_windows_shortcuts.py" (
  echo Creating Windows shortcuts...
  ".venv\Scripts\python.exe" scripts\create_windows_shortcuts.py
  if errorlevel 1 (
    echo.
    echo Setup finished, but shortcuts could not be created.
    echo You can run Create_Shortcuts.bat later.
  )
)

echo.
echo Setup complete.
echo You can now double-click Excel AI Controller.bat.
echo.
pause
exit /b 0
